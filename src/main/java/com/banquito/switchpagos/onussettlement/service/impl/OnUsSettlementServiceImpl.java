...
public class OnUsSettlementServiceImpl implements OnUsSettlementService {
    ...
    public void processOnUsLine(PaymentLineRoutedOnUsEvent event) {
        ...
        try {
            ...
            if (Boolean.TRUE.equals(exception.isFunctionalRejection())) {
                settlementMapper.applyCoreRejected(
                        settlement,
                        attempt,
                        toRejectedResponse(exception),
                        failureTime);
            } else {
    ...
    private CoreOnUsSettlementResponse toRejectedResponse(CoreBankingClientException exception) {
        CoreOnUsSettlementResponse response = new CoreOnUsSettlementResponse();
        response.setStatus("HTTP_" + (exception.getHttpStatus() == null ? "FUNCTIONAL_REJECTION" : exception.getHttpStatus()));
        response.setCoreTransactionId(null);
        response.setMessage(limitMessage(exception.getMessage()));
        return response;
    }
